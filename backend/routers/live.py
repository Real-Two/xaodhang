...
import fetch_rainfall_openmeteo   # no GEE needed for rainfall — direct HTTP
...
try:
    ...
    print(f"[WARN] live.py: GEE (satellite) unavailable: {_e}")
...
def _do_predict(query: LiveQuery, db: Session, model: StructuralRiskModel) -> dict:
    ...
    if zone is None:
        if not _GEE_AVAILABLE:
            raise HTTPException(
                status_code=503,
                detail="Live prediction unavailable: GEE satellite not configured.",
            )
        try:
        ...
        # Rainfall via Open-Meteo — no GEE, no key, actual current data
        try:
