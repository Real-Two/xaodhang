...
# First delete the 5 fake zones
existing = requests.get("http://127.0.0.1:8000/zones").json()
print(f"Existing zones: {len(existing)} (these are the fake demo ones)")

# Add real zones
for z in real_ner_zones:
    r = requests.post("http://127.0.0.1:8000/zones", json=z)
    print(f"{'OK' if r.status_code == 200 else 'FAIL'} — {z['name']}")

print(f"\nDone. Total zones now: {len(requests.get('http://127.0.0.1:8000/zones').json())}")
